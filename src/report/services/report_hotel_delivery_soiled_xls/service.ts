...
import { BuiltSheet, ReportParams } from "./types";
    ...
    if (!hasUserOAuth()) {
      throw new Error(
        "โฟลเดอร์ Drive เป็น My Drive ส่วนตัว — Service Account สร้างไฟล์ไม่ได้\n" +
          ...
          "4. รัน npm run auth-google แล้ว login Gmail\n" +
          "5. restart server"
      );
    }
  ...
  const { spreadsheetId, sheetIds } = await withGoogleRetry(
    () =>
      createSpreadsheet(
        sheets,
        drive,
        `Report Hotel Delivery Soiled ${params.startDate}`,
        sheetTitles
      ),
...
async function generate(params: ReportParams): Promise<ReportResult> {
  ...
  try {
    ...
    if (isDriveQuotaError(error)) {
      console.warn("Google Drive quota = 0 → ใช้ local xlsx แทน");
      return exportViaLocal(allSheets, fileName, outputPath);
    }
    throw error;
  }
...
export const hotelDeliverySoiledReport: ReportService = {
  id: "report_hotel_delivery_soiled_xls",
  route: "/report_hotel_delivery_soiled_xls",
  description: "รายงาน โรงแรมส่งผ้าเปื้อน",
  exampleUrl:
    "/report_hotel_delivery_soiled_xls?date=22-06-2026&lg=th&hotel_code=GNK",
