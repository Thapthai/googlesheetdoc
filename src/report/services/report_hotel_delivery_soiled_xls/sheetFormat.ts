...
export function buildFormatRequests(
  ...
  const borderStartRow =
    sheet.headerRange?.startRow ?? sheet.dataStartRow;

  if (sheet.dataEndRow >= borderStartRow) {
    requests.push({
      repeatCell: {
        range: {
          sheetId,
          startRowIndex: borderStartRow,
  ...
  const colAWidth = sheet.layout === "daily" ? 260 : 280;
  const otherWidth = sheet.layout === "daily" ? 120 : 100;
  ...
  if (sheet.totalCols > 1) {
    requests.push({
      updateDimensionProperties: {
        ...
        properties: { pixelSize: otherWidth },
        fields: "pixelSize",
      },
    });
  }
  ...
  requests.push({
    updateSheetProperties: {
      properties: {
        sheetId,
        gridProperties: {
          frozenRowCount: sheet.freezeRowCount,
          frozenColumnCount: 0,
        },
      },
